...
public class VehicleSharingPackage {
    ...
    public BigDecimal computeTimeAssociatedPriceChange(int remainingFreeSeconds, int totalDrivingTimeInSeconds, int timeChangeInSeconds){
        ...
        if (timeChangeInSeconds > 0) {
            if (totalDrivingTimeInSeconds <= packageTimeLimitInSeconds+freeSeconds) {
                priceChange = (new BigDecimal((double)timeChangeInSeconds/secondsPerTimeTickInPackage)).multiply(drivingPricePerTimeTickInPackage);
            } else {
                priceChange = (new BigDecimal((double)timeChangeInSeconds/secondsPerTimeTickInPackageExceeded)).multiply(drivingPricePerTimeTickInPackageExceeded);
            }
        }
        return priceChange;
    }
